...
class ReverseTrackNavNode(object):
    def __init__(self):
        ...
        self.pre_stop_align_distance_m = max(0.0, float(rospy.get_param('~pre_stop_align_distance_m', 0.15)))
        # 停车前不要把横差修正完全退到 0。保留一点横差修正，
        # 避免航向回正后横差又变大。
        self.pre_stop_cte_keep_ratio = clamp(float(rospy.get_param('~pre_stop_cte_keep_ratio', 0.35)), 0.0, 1.0)
        self.heading_deadband = math.radians(float(rospy.get_param('~heading_deadband_deg', 0.4)))
        # 弱横差积分：只用于消除 2~4 cm 级小稳态横差。
        # 积分项加到“横差航向修正角”里，不直接打角速度；并且有限幅、泄漏、停车不积分。
        self.enable_cross_track_integral = rospy.get_param('~enable_cross_track_integral', True)
        self.k_cross_track_i = float(rospy.get_param('~k_cross_track_i', 0.02))
        self.cross_track_integral_limit = abs(float(rospy.get_param('~cross_track_integral_limit', 0.50)))
        self.max_integral_yaw_correction = math.radians(abs(float(rospy.get_param('~max_integral_yaw_correction_deg', 0.8))))
        self.integral_active_max_cross_track = abs(float(rospy.get_param('~integral_active_max_cross_track', 0.12)))
        self.integral_active_max_heading = math.radians(abs(float(rospy.get_param('~integral_active_max_heading_deg', 3.0))))
        self.integral_leak_rate = max(0.0, float(rospy.get_param('~integral_leak_rate', 0.03)))
        ...
        self.pause_smooth_stop_s = max(0.0, float(rospy.get_param('~pause_smooth_stop_s', 0.05)))
        self.pause_smooth_start_s = max(0.0, float(rospy.get_param('~pause_smooth_start_s', 0.05)))
        ...
        self.last_integral_yaw_correction = 0.0
    ...
    def update_cross_track_integral(self, cross_track_error, base_heading_error, now, active):
        """弱横差积分，消除小稳态横差。

        只在正常巡航阶段 active=True 时积分；停车/起步/刹车阶段不积分。
        积分带泄漏和限幅，避免上方丝杠左右移动后旧积分长期残留。
        """
        if not self.enable_cross_track_integral:
            self.cross_track_integral = 0.0
        ...
        if self.last_integral_time is None:
            self.last_integral_time = now
            self.last_integral_yaw_correction = clamp(
                -self.k_cross_track_i * self.cross_track_integral,
                -self.max_integral_yaw_correction,
                self.max_integral_yaw_correction)
            return self.last_integral_yaw_correction

        dt = max(0.0, min(0.50, (now - self.last_integral_time).to_sec()))
        self.last_integral_time = now

        # 泄漏积分：慢慢忘掉旧偏差，适应丝杠左右切换后的扰动变化。
        if dt > 0.0 and self.integral_leak_rate > 0.0:
            leak = max(0.0, 1.0 - self.integral_leak_rate * dt)
            self.cross_track_integral *= leak

        can_integrate = (active and
                         abs(cross_track_error) <= self.integral_active_max_cross_track and
                         abs(base_heading_error) <= self.integral_active_max_heading)
        if can_integrate and dt > 0.0:
            self.cross_track_integral += cross_track_error * dt

        self.cross_track_integral = clamp(
            self.cross_track_integral,
            -self.cross_track_integral_limit,
            self.cross_track_integral_limit)

        integral_yaw = -self.k_cross_track_i * self.cross_track_integral
        integral_yaw = clamp(integral_yaw, -self.max_integral_yaw_correction, self.max_integral_yaw_correction)
        self.last_integral_yaw_correction = integral_yaw
        return integral_yaw

    def compute_spray_line_control(self, desired_yaw, pose_yaw, cross_track_error, dist_to_stop, now, integral_active):
        """喷洒两点模式控制。

        横差只生成一个小的航向偏置角，不再直接作为角速度项。
        距离停车点越近，横差航向偏置越小，但不会完全归零。
        这样可以保证喷洒停车时车头大体一致，同时不把已修回的横差又放出去。
        """
        base_heading_error = wrap_to_pi(desired_yaw - pose_yaw)
        ...
        if self.pre_stop_align_distance_m > 1e-6 and dist_to_stop < self.pre_stop_align_distance_m:
            # 停车前保留一部分横差修正，不完全退到 0，避免刚修小的横差被航向回正抵消。
            align_ratio = clamp(dist_to_stop / self.pre_stop_align_distance_m, 0.0, 1.0)
            align_ratio = max(align_ratio, self.pre_stop_cte_keep_ratio)
            cte_yaw *= align_ratio

        integral_yaw = self.update_cross_track_integral(cross_track_error, base_heading_error, now, integral_active)
        corrected_yaw = wrap_to_pi(desired_yaw + cte_yaw + integral_yaw)
        ...
        return angular_raw, heading_error, cte_yaw, integral_yaw, align_ratio, corrected_yaw
    ...
    def reset_cross_track_integral(self):
        self.cross_track_integral = 0.0
        self.last_integral_time = None
        self.last_integral_yaw_correction = 0.0
    ...
    def stop_auto_with_reason(self, reason, fault_pause=False):
        ...
        self.reset_cross_track_integral()
        self.publish_zero()
        if fault_pause:
    ...
    def control_loop(self, _event):
        with self.lock:
            ...
            integral_yaw_correction = self.last_integral_yaw_correction
            pre_stop_align_ratio = 1.0
            corrected_yaw = desired_yaw

            if self.is_spray_line_mode():
                # 只在巡航阶段积分；刹车、停车保持、起步阶段冻结积分。
                integral_active = (self.pause_state == 'cruise')
                base_angular_z_raw, heading_error, cte_yaw_correction, integral_yaw_correction, pre_stop_align_ratio, corrected_yaw = self.compute_spray_line_control(
                    desired_yaw, pose.yaw, cross_track_error, dist_to_work_stop, now, integral_active)
            else:
            ...
            if not self.should_enable_pause() or dist_goal <= self.pause_skip_near_goal_m:
                ...
                self.publish_status(
                    '自动倒车中 | 模式: {} | 录点: {} | 段号: {} | 段进度: {:.2f} | 段终点距: {:.2f} m | 终点距: {:.2f} m | 横差: {:.2f} m | 航差: {:.1f} deg | 横差航向修正: {:.1f} deg | 积分修正: {:.1f} deg | 停车对齐比例: {:.0f}%'.format(
                        ...
                        math.degrees(cte_yaw_correction), math.degrees(integral_yaw_correction), pre_stop_align_ratio * 100.0))
                return
            ...
            if self.pause_state == 'cruise':
                ...
                if segment_progress_s >= self.next_pause_progress:
                    ...
                    self.publish_cmd(base_linear_x, cruise_angular_z)
                    self.publish_status(
                        '自动倒车中 | 模式: {} | 录点: {} | 段号: {} | 段进度: {:.2f} | 段终点距: {:.2f} m | 终点距: {:.2f} m | 横差: {:.2f} m | 航差: {:.1f} deg | 横差航向修正: {:.1f} deg | 积分修正: {:.1f} deg | 停车对齐比例: {:.0f}% | 下次停车距: {:.2f} m'.format(
                            self.tracking_mode, len(self.recorded_path), self.segment_index, raw_t,
                            dist_seg_end, dist_goal, cross_track_error, math.degrees(heading_error),
                            math.degrees(cte_yaw_correction), math.degrees(integral_yaw_correction), pre_stop_align_ratio * 100.0,
            ...
            if self.pause_state == 'brake':
                ...
                if self.pause_smooth_stop_s <= 1e-6:
                    ratio = 0.0
                else:
            ...
            if self.pause_state == 'start':
                ...
                if self.pause_smooth_start_s <= 1e-6:
                    linear_ratio = 1.0
                else:
