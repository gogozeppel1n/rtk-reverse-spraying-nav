...
class ControllerPanel(object):
    def __init__(self):
        ...
        self.srv_resume_auto = rospy.get_param('~srv_resume_auto', ns + '/resume_auto_reverse')
        ...
        self.root = tk.Tk()
        self.root.title('RTK 两点打点倒车控制器')
        self.root.geometry('820x630')
        self.root.protocol('WM_DELETE_WINDOW', self.on_close)

        self.info_var = tk.StringVar(value=(
            '推荐操作顺序：\n'
            '默认使用：两点打点直线倒车；多点轨迹仅作为备用。\n'
            '本版已加入：停车前角度保持 + 起步先直倒 + 横向纠偏渐进恢复。\n'
            '1. 车停终点，点【记录终点并开始录轨】\n'
            '2. 遥控正向开到起点\n'
            '3. 到起点后点【记录起点并停止录轨】\n'
            '4. 点【启动自动倒车】\n'
            '5. 横向误差过大停车后，人工调回轨迹附近，再点【人工调整后继续倒车】。\n'
            '备用：需要完整轨迹复现时，先点【备用：切换到多点轨迹模式】再重新录制。'
        ))
        ...
        title = tk.Label(self.root, text='RTK 两点打点倒车控制器', font=('Arial', 18, 'bold'))
        title.pack(pady=10)
        ...
        tk.Button(frame, text='两点打点模式（默认）', command=lambda: self.call_service_async(self.srv_set_line), bg='#d7f7d7', **btn_cfg).grid(row=0, column=0, padx=8, pady=8)
        tk.Button(frame, text='备用：多点轨迹模式', command=lambda: self.call_service_async(self.srv_set_path), bg='#d7e9ff', **btn_cfg).grid(row=0, column=1, padx=8, pady=8)
        ...
        tk.Button(frame, text='人工调整后继续倒车', command=lambda: self.call_service_async(self.srv_resume_auto), bg='#c9f2ff', **btn_cfg).grid(row=2, column=1, padx=8, pady=8)
        tk.Button(frame, text='停止自动', command=lambda: self.call_service_async(self.srv_stop_auto), bg='#ffb3b3', **btn_cfg).grid(row=3, column=0, padx=8, pady=8)
        tk.Button(frame, text='保存当前模式轨迹 CSV', command=lambda: self.call_service_async(self.srv_save), **btn_cfg).grid(row=3, column=1, padx=8, pady=8)
        tk.Button(frame, text='清空轨迹', command=lambda: self.call_service_async(self.srv_clear), bg='#ffe4b3', **btn_cfg).grid(row=4, column=0, padx=8, pady=8)
        tk.Button(frame, text='选择并导入 CSV 轨迹', command=self.choose_and_import_csv, bg='#e8d7ff', **btn_cfg).grid(row=4, column=1, padx=8, pady=8)
        tk.Entry(frame, textvariable=self.import_file_var, font=('Arial', 10), width=45).grid(row=5, column=1, padx=8, pady=8, sticky='we')
