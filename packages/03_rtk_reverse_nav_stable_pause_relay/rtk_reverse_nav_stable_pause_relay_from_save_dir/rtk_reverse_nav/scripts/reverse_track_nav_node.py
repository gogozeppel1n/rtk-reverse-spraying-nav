...
class ReverseTrackNavNode(object):
    ...
    def control_loop(self, _event):
        with self.lock:
            ...
            base_angular_z = self.k_heading * heading_error - self.k_cross_track * cross_track_error
            base_angular_z = max(-self.max_angular_speed, min(self.max_angular_speed, base_angular_z))
            ...
            if self.pause_state == 'cruise':
                ...
                if segment_progress_s >= self.next_pause_progress:
                    ...
                    self.publish_status('达到分段停车点，开始减速停车。当前进度 {:.2f} m，下一次停车将在起步完成后重新累计。'.format(segment_progress_s))
                else:
                    self.publish_cmd(base_linear_x, base_angular_z)
                    self.publish_status(
            ...
            if self.pause_state == 'start':
                ...
                if elapsed >= self.pause_smooth_start_s:
                    # 起步完成后，以当前实际进度重新累计下一次停车距离；这是本版核心修复。
                    self.pause_state = 'cruise'
                    self.next_pause_progress = segment_progress_s + self.pause_distance_m
                    self.publish_cmd(base_linear_x, base_angular_z)
                    self.publish_status('柔顺起步完成，继续连续倒车。下一次停车目标进度 {:.2f} m。'.format(self.next_pause_progress))
                    return
                linear_x = base_linear_x * ratio
                # 起步阶段限制角速度幅值和变化率，但不改变原始算法的方向。
                angular_limit = max(0.0, self.pause_start_angular_limit)
                angular_z = max(-angular_limit, min(angular_limit, base_angular_z)) * ratio
                angular_z = self.rate_limit_angular(angular_z, now, self.max_pause_angular_accel)
                self.publish_cmd(linear_x, angular_z)
                self.publish_status('柔顺起步中 {:.0f}% | 模式: {} | 段号: {} | 终点距: {:.2f} m | 横差: {:.2f} m | 航差: {:.1f} deg'.format(
                    ratio * 100.0, self.tracking_mode, self.segment_index, dist_goal, cross_track_error, math.degrees(heading_error)))
                return
