...
本版默认 `tracking_mode: line`，即两点打点直线倒车模式。正常现场作业时只需要记录两个点：

1. 车停在倒车终点，点击“记录终点并开始录轨”；
2. 人工遥控正向开到倒车起点；
3. 点击“记录起点并停止录轨”；
4. 点击“启动自动倒车”。

多点轨迹模式 `tracking_mode: path` 仍保留作为备用。需要完整轨迹复现时，先切换到“备用：多点轨迹模式”，再重新录制轨迹。

注意：本版已将 launch 默认参数与 yaml 统一：`reverse_speed=0.15`，`pause_distance_m=0.30`，`pause_smooth_stop_s=0.35`，`pause_smooth_start_s=0.50`。
